...
export function StudentDialog({ student, trigger, open, onOpenChange, onSuccess, branches, courses, userRole }: StudentDialogProps) {
    ...
    const [branchId, setBranchId] = useState(student?.branchId?.toString() || "");

    // MultiSelect requires string array for values
    ...
    useEffect(() => {
        if (effectiveOpen) {
            if (!student) {
                ...
                setSelectedCourseIds(student.enrollments?.map(e => e.courseId.toString()) || []);
            }
        }
